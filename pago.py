...
class pagar():
    ...
    def __init__(self, root, val, ser, dia):
        global frame_pago
        frame_pago = Toplevel(root)

        frame_pago.configure(bg="#27AEF2")
        ...
        inbox_frame = LabelFrame(frame_pago, bg="#53CDB8", width=300, height=80, padx=4, pady=5)
        inbox_frame.grid(row=1, column=0)
        inbox_frame_val = LabelFrame(frame_pago, bg="#53CDB8", width=300, height=80, padx=4, pady=5)
        inbox_frame_val.grid(row=2, column=0)

        inbox_frame_ab = LabelFrame(frame_pago, bg="#53CDB8", width=300, height=80, padx=4, pady=5)
        inbox_frame_ab.grid(row=4, column=0)

        button_frame = LabelFrame(frame_pago, bg="#53CDB8")
        button_frame.grid(row=3, column=0)
        Label(inbox_frame, text='Asegura tu reservacion', bg="#53CDB8", font=("Comic Sans MS", "30", "bold")).grid(
            row=0,
            column=0)
        Label(inbox_frame_val, text='Valor a cancelar: ', bg="#53CDB8", font=("Comic Sans MS", "11", "bold")).grid(
            row=1,
            column=0)

        inbox_servicio = Entry(inbox_frame_val, textvariable=total, font=("Comic Sans MS", "11", "normal"), width=15)
        inbox_servicio.grid(row=1, column=2, padx=30, pady=5)
        inbox_servicio.focus()

        Label(inbox_frame, text='Selecciona metodo de pago', bg="#53CDB8", font=("Comic Sans MS", "20", "bold")).grid(
            row=2,
            column=0)

        pagarefec_button = Button(button_frame, command=lambda: continuar(), text='Efectivo', width=30)
        pagarefec_button.configure(bg="#FFBB20", cursor='hand2', font=("Comic Sans MS", "10", "normal"))
        ...
        pagartar_button.configure(bg="#FFBB20", cursor='hand2', font=("Comic Sans MS", "10", "normal"))
        ...
        def continuar():
            metodo = "Efectivo"
            metodopago(metodo)
            Label(inbox_frame_ab, text='Cantidad abono', bg="#53CDB8",
                  font=("Comic Sans MS", "20", "bold")).grid(row=12, column=0)
            inbox_abonar = Entry(inbox_frame_ab, textvariable=abono, font=("Comic Sans MS", "11", "normal"),
                                 width=50)
        ...
        def valorpago():
            ...
            total = (float(val) * int(dia)) + float(ser)
            inbox_servicio.insert(tk.END, str(total))
            print(total)
            return total
